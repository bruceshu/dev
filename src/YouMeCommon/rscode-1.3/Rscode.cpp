...
namespace youmecommon {
	int CRscode::gexp[512] = { 0 };
	int CRscode::glog[256] = { 0 };

	void CRscode::Modified_Berlekamp_Massey(void)
	{
		int n, L, L2, k, d, i;
	...
	void CRscode::init_gamma(int gamma[])
	{
		int e, tmp[MAXDEG];
	...
	void CRscode::add_polys(int dst[], int src[])
	{
		int i;
		for (i = 0; i < MAXDEG; i++) dst[i] ^= src[i];
	}

	void CRscode::scale_poly(int k, int poly[])
	{
		int i;
		for (i = 0; i < MAXDEG; i++) poly[i] = gmult(k, poly[i]);
	}

	void CRscode::mul_z_poly(int src[])
	{
		int i;
	...
	int CRscode::compute_discrepancy(int lambda[], int S[], int L, int n)
	{
		int i, sum = 0;
	...
	void CRscode::compute_modified_omega()
	{
		int i;
	...
	void CRscode::build_codeword(unsigned char msg[], int nbytes, unsigned char dst[])
	{
		int i;
	...
	void CRscode::init_exp_table(void)
	{
		int i, z;
	...
	void CRscode::mult_polys(int dst[], int p1[], int p2[])
	{
		int i, j;
		int tmp1[MAXDEG * 2];
	...
	void CRscode::compute_genpoly(int nbytes, int genpoly[])
	{
	...
	int CRscode::genPoly[MAXDEG * 2] = { 0 };
	CRscode::CRscode()
	{
	...
	void CRscode::encode_data(unsigned char msg[], int nbytes, unsigned char dst[])
	{
	...
	void CRscode::decode_data(unsigned char data[], int nbytes)
	{
		int i, j, sum;
		for (j = 0; j < NPAR; j++) {
			sum = 0;
	...
	int CRscode::check_syndrome(void)
	{
		int i, nz = 0;
		for (i = 0; i < NPAR; i++) {
			if (synBytes[i] != 0) {
	...
	int CRscode::correct_errors_erasures(unsigned char codeword[], int csize, int nerasures, int erasures[])
	{
		int r, i, j, err;
