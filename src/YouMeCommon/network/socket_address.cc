...
#include "YouMeCommon/network/ipv6_address_utils.h"
...
namespace youmecommon
{
	...
	void  socket_address::__init(const sockaddr* _addr) {
		...
		if (AF_INET == _addr->sa_family) {
			...
			if (IN6_IS_ADDR_NAT64(&addr_.in6.sin6_addr)) {
				...
				sockaddr_in addr = { 0 };
				addr.sin_family = AF_INET;
				addr.sin_addr.s_addr = addr_.in6.sin6_addr.s6_addr[3];
	...
	bool socket_address::fix_current_nat64_addr() {
		...
		if (AF_INET6 == addr_.sa.sa_family && 0 != strncmp("::FFFF:", ip_, 7)) {
			//更新addr_, ip_, url_
			//		if (is_update) {
			in6_addr nat64_v6_addr;
			ret = ConvertV4toNat64V6(*(struct in_addr*)(&(addr_.in6.sin6_addr.s6_addr[3])), nat64_v6_addr);
			//xdebug2(TSF"ret =%_, ip_=%_, nat64_v6_addr = %_", ret, ip_, strutil::Hex2Str((char*)&(nat64_v6_addr.s6_addr32), 16));
			if (ret) {
				memcpy((char*)&(addr_.in6.sin6_addr.s6_addr), (char*)&(nat64_v6_addr.s6_addr), 16);
				socket_inet_ntop(AF_INET6, &(addr_.in6.sin6_addr), ip_, sizeof(ip_));
				//-----把ip_转为更易读的v6 ip形式---//
				if (0 == youmecommon::strnicmp(kWellKnownNat64Prefix, ip_, 9)) {
					sockaddr_in addr_v4 = { 0 };
					addr_v4.sin_family = AF_INET;
					addr_v4.sin_addr.s_addr = addr_.in6.sin6_addr.s6_addr[3];
	...
	const char* socket_address::ip() const {
		if (AF_INET == addr_.sa.sa_family) {
			...
			if (0 == youmecommon::strnicmp("::FFFF:", ip_, 7))
				return ip_ + 7;
			else if (0 == youmecommon::strnicmp(kWellKnownNat64Prefix, ip_, 9))
				return ip_ + 9;
			else
				return ip_;
		}
