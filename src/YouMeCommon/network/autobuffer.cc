...
#define min(a, b) (((a) < (b)) ? (a) : (b))
	...
	void AutoBuffer::__FitSize(size_t _len) {
		if (_len > capacity_) {
			...
			if (NULL == p) {
				//ASSERT2(p, "_len=%" PRIu64 ", m_nMallocUnitSize=%" PRIu64 ", nMallocSize=%" PRIu64", m_nCapacity=%" PRIu64, (uint64_t)_len, (uint64_t)malloc_unitsize_, (uint64_t)mallocsize, (uint64_t)capacity_);
				free(parray_);
			}
