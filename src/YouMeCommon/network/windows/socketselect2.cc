...
namespace youmecommon
{
	...
	static DWORD __SO_RCVTIMEO(SOCKET _sock) {
		...
		getsockopt(_sock, SOL_SOCKET, SO_RCVTIMEO, &optval, &optlen);
	...
	static void __SO_RCVTIMEO(SOCKET _sock, DWORD _ms) {
		DWORD optval = _ms;
		int optlen = sizeof(optval);
		setsockopt(_sock, SOL_SOCKET, SO_RCVTIMEO, (const char*)&optval, optlen);
	...
	SocketBreaker::SocketBreaker()
		: m_broken(false)
		, m_create_success(true), m_exception(0), m_event(WSA_INVALID_EVENT){
		ReCreate();
	}
	...
	int SocketSelect::Select(int _msec) {
		...
		for (std::map<SOCKET, int>::iterator it = m_filter_map.begin(); it != m_filter_map.end(); ++it) {
			...
			WSAEventSelect(it->first, eventarray[index], it->second);
			++index;
		}
