#include "YouMeCommon/network/nat64_prefix_util.h"
#include "YouMeCommon/network/local_ipstack.h"
#include "YouMeCommon/network/unix_socket.h"
#include "YouMeCommon/network/strutil.h"
#include "YouMeCommon/network/platform_comm.h"
...
#ifdef __APPLE__
	if (youmecommon::publiccomponent_GetSystemVersion() >= 9.2f) is_iOS_above_9_2 = true;
#endif
	size_t suffix_zero_count = GetSuffixZeroCount((uint8_t*)_replaced_nat64_addr, sizeof(struct in6_addr));
	if (0 != suffix_zero_count) {
		//xwarn2(TSF"suffix_zero_count=%_, _replaced_nat64_addr=%_", suffix_zero_count,	strutil::Hex2Str((char*)_replaced_nat64_addr, sizeof(struct in6_addr)));
	}
	bool is_valid = false;
...
bool ConvertV4toNat64V6(const struct in_addr& _v4_addr, struct in6_addr& _v6_addr) {
	//xdebug_function();
	if (youmecommon::ELocalIPStack_IPv6 != youmecommon::local_ipstack_detect()) {
...
#ifdef __APPLE__
	if (youmecommon::publiccomponent_GetSystemVersion() >= 9.2f) {//higher than iOS9.2
			...
			if (AF_INET6 == res->ai_family) {
                ...
                if (youmecommon::publiccomponent_GetSystemVersion() >= 9.2f) { //higher than iOS9.2
					...
					if (IsNat64AddrValid((struct in6_addr*)&(((sockaddr_in6*)res->ai_addr)->sin6_addr))) {
						...
						memcpy((char*)&_v6_addr, (char*)&((((sockaddr_in6*)res->ai_addr)->sin6_addr).s6_addr), 16);
						...
						//xerror2(TSF"Nat64 addr invalid, =%_", strutil::Hex2Str((char*)&(((sockaddr_in6*)res->ai_addr)->sin6_addr), 16));
						ret = false;
					}
#ifdef __APPLE__
				}
...
bool  GetNetworkNat64Prefix(struct in6_addr& _nat64_prefix_in6) {
	...
	bool ret = false;
#ifdef __APPLE__
	if (youmecommon::publiccomponent_GetSystemVersion() >= 9.2f) {
	...
	if (error == 0) {
		for (res = res0; res; res = res->ai_next) {
			...
			if (AF_INET6 == res->ai_family) {
				memcpy((char*)&(_nat64_prefix_in6.s6_addr), (char*)&((((sockaddr_in6*)res->ai_addr)->sin6_addr).s6_addr), 12);
