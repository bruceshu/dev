...
#include "YouMeCommon/network/unix/getifaddrs.h"
#include "YouMeCommon/network/unix/getgateway.h"
#include "YouMeCommon/network/unix/getdnssvraddrs.h"
...
{
#if defined(OS_OSX)
    YouMeApplication_OSX SystemProvider;
#else
    YouMeApplication_iOS SystemProvider;
#endif
    std::string systemVersion = SystemProvider.getSystemVersion();
    double ver = 0.0;
    if (systemVersion != "") {
        std::vector<XString> spices;
        CStringUtil::splitString(UTF8TOXString(systemVersion), __XT("."), spices);
        if (spices.size()>1) {
            std::string tmp = spices[0] + "." + spices[1];
            ver = CStringUtilT<char>::str_to_double(tmp.c_str());
            return (float)ver;
        }
    }
