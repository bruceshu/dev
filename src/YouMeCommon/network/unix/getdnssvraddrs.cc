...
void getdnssvraddrs(std::vector<youmecommon::socket_address>& dnsServers) {
    char buf1[PROP_VALUE_MAX];
    char buf2[PROP_VALUE_MAX];
    __system_property_get("net.dns1", buf1);
    __system_property_get("net.dns2", buf2);
    dnsServers.push_back(youmecommon::socket_address(buf1, 0));  // 主DNS
    dnsServers.push_back(youmecommon::socket_address(buf2, 0));  // 备DNS
}
...
void getdnssvraddrs(std::vector<youmecommon::socket_address>& _dnssvraddrs) {
    ...
    for (int i = 0; i < count; ++i) {
        if (AF_INET == addrs[i].sin.sin_family) {
            _dnssvraddrs.push_back(youmecommon::socket_address(addrs[i].sin));
        } else if (AF_INET6 == addrs[i].sin.sin_family) {
            _dnssvraddrs.push_back(youmecommon::socket_address(addrs[i].sin6));
        }
