...
typedef youmecommonrapidjson::GenericValue<youmecommonrapidjson::UTF8<char>> JSONValue;
...
public:
  ...
  DictPtr ParseDict(const JSONValue& doc) {
    ...
    if (type == "group") {
      ...
      for (youmecommonrapidjson::SizeType i = 0; i < docs.Size(); i++) {
        if (docs[i].IsObject()) {
...
ConverterPtr Config::NewFromString(const string& json,
  ...
  youmecommonrapidjson::Document doc;
