...
#define SSLSESSION_SHARED(data) (data->share &&                        \
	...
	unsigned int Curl_rand(youmecommon::SessionHandle *data)
	{
		unsigned int r = 0;
	...
	int Curl_ssl_init(void)
	{
		...
		return youmecommon::Curl_axtls_init();
	}
		...
		Curl_ssl_connect(struct connectdata *conn, int sockindex)
	{
		...
		result = Curl_axtls_connect(conn, sockindex);
...
//#endif /* non-blocking connect support */
	...
	void Curl_ssl_kill_session(struct curl_ssl_session *session)
	{
		if(session->sessionid) {
			...
			Curl_axtls_session_free(session->sessionid);
	...
	{
		if (Curl_axtls_shutdown(conn, sockindex))
	...
	{
		return youme_curlssl_set_engine(data, engine);
	}
	...
	{
		return youme_curlssl_set_engine_default(data);
	}

	/* Return list of OpenSSL crypto engine names. */
	struct curl_slist *Curl_ssl_engines_list(struct SessionHandle *data)
	{
		return youme_curlssl_engines_list(data);
	}
	...
	CURLcode Curl_ssl_initsessions(struct SessionHandle *data, size_t amount)
	{
		...
		session = (youmecommon::curl_ssl_session*)calloc(amount, sizeof(struct curl_ssl_session));
		if(!session)
			return CURLE_OUT_OF_MEMORY;
	...
	{
		return Curl_axtls_version(buffer, size);
	}
	...
	{
		return Curl_axtls_check_cxn(conn);
	}

	bool Curl_ssl_data_pending(const struct connectdata *conn,
		int connindex)
	{
		return youme_curlssl_data_pending(conn, connindex);
	}
	...
	CURLcode Curl_ssl_init_certinfo(struct SessionHandle *data, int num)
	{
		...
		table =(youmecommon::curl_slist**) calloc((size_t) num, sizeof(struct curl_slist *));
		if(!table)
			return CURLE_OUT_OF_MEMORY;
	...
	int Curl_ssl_random(struct SessionHandle *data,
		...
		size_t length)
	{
		return Curl_axtls_random(data, entropy, length);
	}
		...
		unsigned char **der, size_t *der_len)
	{
		...
		end_pos = (char*)(pem + pem_count, "\n-----END PUBLIC KEY-----");
		if(!end_pos)
			return CURLE_BAD_CONTENT_ENCODING;

		pem_len = end_pos - pem;

		stripped_pem = (char*)(pem_len - pem_count + 1);
		if(!stripped_pem)
			return CURLE_OUT_OF_MEMORY;
