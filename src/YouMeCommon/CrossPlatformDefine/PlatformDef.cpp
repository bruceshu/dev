...
{
	//包括0结尾
	int iSrcElementCount = XStrLen(pszSrc)+1;
	if (iDestElementCount > iSrcElementCount)
	{
		iDestElementCount = iSrcElementCount;
	}

	memcpy(pszDest, pszSrc, sizeof(XCHAR)*(iDestElementCount-1));
	pszDest[iDestElementCount-1] = 0;
	//返回的是实际拷贝的个数，不包括0
	return iDestElementCount-1;
}
