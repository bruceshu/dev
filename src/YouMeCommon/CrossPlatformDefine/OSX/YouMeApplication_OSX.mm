...
YouMeApplication_OSX::YouMeApplication_OSX ()
{
}
...
//    [task launch];
//
//    NSArray * args2 = [NSArray arrayWithObjects:@"/IOPlatformUUID/ { split($0, line, \"\\\"\"); printf(\"%s\", line[4]); }", nil];
...
XString YouMeApplication_OSX::getCpuChip ()
{
    return "";
}
...
XString YouMeApplication_OSX::getCachePath()
{
    NSArray *path = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
    NSString *cachePath = [path objectAtIndex:0];
    
    XString packageName = getPackageName();
    
    NSString *nsPackageName = [[NSString alloc] initWithUTF8String:packageName.c_str()];
    
    NSString *myCacheDir = [cachePath stringByAppendingPathComponent:nsPackageName];
        
    if (![[NSFileManager defaultManager] fileExistsAtPath:myCacheDir]) {
        [[NSFileManager defaultManager]createDirectoryAtPath:myCacheDir withIntermediateDirectories:YES attributes:nil error:nil];
    }
    
    const char *path_cstr = [[NSFileManager defaultManager] fileSystemRepresentationWithPath:myCacheDir];
