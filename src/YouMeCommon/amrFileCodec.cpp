...
#include <YouMeCommon/wavresample/PCMResampleTo8K.h>
...
{
	//重采样成8K .内部会自己判断
	bool bUseResample = false;
	XString strResamplePath = strWAVEFilename + __XT(".tmp");
	bUseResample = youmecommon::CPCMResampleTo8K::ConvertTo8K(strWAVEFilename, strResamplePath);
	if (!bUseResample)
	{
		strResamplePath = strWAVEFilename;
	}
	...
	if (0 != fpwave.LoadFile(strResamplePath, youmecommon::CXFile::Mode_OpenExist_ReadOnly))
	{
		return 0;
	}
	...
	{
		//删掉临时文件
		youmecommon::CXFile::remove_file(strResamplePath);
	}
...
int ReadAMRFrame(youmecommon::CXFile& fpamr, unsigned char frameBuffer[], int stdFrameSize, unsigned char stdFrameHeader)
{
	...
	while(1)
	{
		fpamr.Read((byte*)&frameHeader, sizeof(unsigned char));
		if (fpamr.IsEnd()) return 0;
    ...
    fpamr.Read((byte*)&(frameBuffer[1]), (stdFrameSize - 1)*sizeof(unsigned char));
	if (fpamr.IsEnd()) return 0;
...
//	WriteWAVEFileHeader(fpwave, nFrameCount);
	...
	if (1 != ReadAMRFrameFirst(fpamr, amrFrame, &stdFrameSize, &stdFrameHeader))
    {
        return 0;
    }
